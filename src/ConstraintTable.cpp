...
void ConstraintTable::buildPeko(const CBSNode& node, int agent)
{
	auto curr = &node;
	while (curr->parent != nullptr)
	{
		if (curr->ag_constraints.find(agent) != curr->ag_constraints.end())  // there is a constraint at CT node curr
		{
			for (const list<Constraint>& ag_con : curr->ag_constraints.at(agent))
			{
				int a, x, y, t;
				constraint_type type;
				tie(a, x, y, t, type) = ag_con.front();
				switch (type)
				{
				case constraint_type::VERTEX:
					if (a == agent)
					{
						for (const auto& constraint : ag_con) // we might have multiple vertex constraints generated by mutex propagation
						{
							tie(a, x, y, t, type) = constraint;
							insert2CT(x, t, t + 1);
						}
